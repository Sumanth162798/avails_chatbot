# avails_bot.py — MVP (Jounce fixed, sums after filters, intents)
...
- If text says "gaming"/"non gaming", add a Vertical filter accordingly.
...
def apply_macros(df: pd.DataFrame, macros: List[str]) -> pd.DataFrame:
    ...
    if "premium" in macros:
        ...
        if "Jounce Media" in d.columns:
            ...
            d = d[~bad_mask]
        # Optional stricter toggles can be turned on later via config
        # (e.g., require SDK, exclude MA). Left off per your directive.
...
user_input = st.text_input("Ask e.g. 'Premium rewarded gaming apps in United States on SDK, Android; include Primary Category'")
...
if user_input:
    ...
    with st.expander("Debug: key counts"):
        ...
        if "Vertical" in df.columns: st.write("Vertical:", df["Vertical"].value_counts(dropna=False))
    ...
    # Hard-enforce vertical/premium/rewarded from text
    t = (user_input or "").lower()
    # Vertical
    q["filters"] = [f for f in (q.get("filters") or []) if f.get("column","").strip().lower() != "vertical"]
    if re.search(r"\bnon[-_\s]?gaming\b", t):
        q.setdefault("filters", []).append({"column":"Vertical","op":"equals","value":"Non Gaming"})
    elif re.search(r"\bgaming\b", t):
        q.setdefault("filters", []).append({"column":"Vertical","op":"equals","value":"Gaming"})
    # Macros
    macs = set(q.get("macros") or [])
    if re.search(r"\bpremium\b|\bsafe\b", t): macs.add("premium")
    if re.search(r"\bgreen\b", t): macs.add("green")
    if re.search(r"\blocal apps?\b", t): macs.add("local_apps")
    q["macros"] = list(macs)
    # Rewarded
    if re.search(r"\brewarded( video)?\b", t):
    ...
    if intent == "count":
        ...
        st.stop()
    if intent == "breakdown":
        # If user hinted formats (banner/fullscreen/other), still show general breakdown;
        # For custom 3-bucket split you can add a format bucketing later.
        answer_breakdown(g)
        st.download_button("Download CSV", g.to_csv(index=False).encode("utf-8"), "avails_app_level.csv", "text/csv")
        st.stop()
    if intent == "summary":
